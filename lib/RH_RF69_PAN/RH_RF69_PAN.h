...
#ifndef RH_RF69_PAN_h
#define RH_RF69_PAN_h
...
///
class RH_RF69_PAN : public RHSPIDriver
{
public:
...
//  Test,
    ...
    RH_RF69_PAN(uint8_t slaveSelectPin = SS, uint8_t interruptPin = 2, RHGenericSPI& spi = hardware_spi);
    ...
    /// Sets the PAN header to be sent in all subsequent messages
    /// \param[in] pan The new PAN header value
    void setHeaderPan(uint8_t pan);

    /// Returns the PAN header of the last received message
    /// \return The PAN header
    uint8_t headerPan();
...
protected:
    ...
    static RH_RF69_PAN*     _deviceForInterrupt[];
    ...
    // PAN support
    volatile uint8_t _rxHeaderPan;
    uint8_t _txHeaderPan;
    uint8_t _thisPan;
};
