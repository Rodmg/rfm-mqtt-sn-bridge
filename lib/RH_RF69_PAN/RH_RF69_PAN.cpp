...
// Modified by Rodrigo Méndez 2016 (added PAN support)
...
RH_RF69_PAN* RH_RF69_PAN::_deviceForInterrupt[RH_RF69_NUM_INTERRUPTS] = {0, 0, 0};
uint8_t RH_RF69_PAN::_interruptCount = 0; // Index into _deviceForInterrupt for next device
...
PROGMEM static const RH_RF69_PAN::ModemConfig MODEM_CONFIG_TABLE[] =
{
...
RH_RF69_PAN::RH_RF69_PAN(uint8_t slaveSelectPin, uint8_t interruptPin, RHGenericSPI& spi)
    :
...
void RH_RF69_PAN::setIdleMode(uint8_t idleMode)
{
    _idleMode = idleMode;
}

bool RH_RF69_PAN::init()
{
    if (!RHSPIDriver::init())
  return false;
...
void RH_RF69_PAN::handleInterrupt()
{
...
void RH_RF69_PAN::readFifo()
{
  ...
  _rxHeaderPan   = _spi.transfer(0);

  // Check addressing
  if (_promiscuous ||
      (_rxHeaderTo == _thisAddress && _rxHeaderPan == _thisPan ) ||
      (_rxHeaderTo == RH_BROADCAST_ADDRESS && _rxHeaderPan == _thisPan) )
  {
...
void RH_RF69_PAN::isr0()
{
    if (_deviceForInterrupt[0])
  _deviceForInterrupt[0]->handleInterrupt();
}
void RH_RF69_PAN::isr1()
{
    if (_deviceForInterrupt[1])
  _deviceForInterrupt[1]->handleInterrupt();
}
void RH_RF69_PAN::isr2()
{
    if (_deviceForInterrupt[2])
  _deviceForInterrupt[2]->handleInterrupt();
}

int8_t RH_RF69_PAN::temperatureRead()
{
...
bool RH_RF69_PAN::setFrequency(float centre, float afcPullInRange)
{
    // Frf = FRF / FSTEP
...
void RH_RF69_PAN::setOpMode(uint8_t mode)
{
...
void RH_RF69_PAN::setTxPower(int8_t power)
{
    _power = power;
...
void RH_RF69_PAN::setModemRegisters(const ModemConfig* config)
{
...
bool RH_RF69_PAN::setModemConfig(ModemConfigChoice index)
{
    if (index > (signed int)(sizeof(MODEM_CONFIG_TABLE) / sizeof(ModemConfig)))
        return false;

    ModemConfig cfg;
    memcpy_P(&cfg, &MODEM_CONFIG_TABLE[index], sizeof(RH_RF69_PAN::ModemConfig));
    setModemRegisters(&cfg);

    return true;
}

void RH_RF69_PAN::setPreambleLength(uint16_t bytes)
{
    spiWrite(RH_RF69_REG_2C_PREAMBLEMSB, bytes >> 8);
    spiWrite(RH_RF69_REG_2D_PREAMBLELSB, bytes & 0xff);
}

void RH_RF69_PAN::setSyncWords(const uint8_t* syncWords, uint8_t len)
{
...
void RH_RF69_PAN::setEncryptionKey(uint8_t* key)
{
    if (key)
    {
...
bool RH_RF69_PAN::recv(uint8_t* buf, uint8_t* len)
{
    if (!available())
  return false;
...
bool RH_RF69_PAN::send(const uint8_t* data, uint8_t len)
{
...
uint8_t RH_RF69_PAN::maxMessageLength()
{
    return RH_RF69_MAX_MESSAGE_LEN;
}

bool RH_RF69_PAN::printRegister(uint8_t reg)
{
...
bool RH_RF69_PAN::printRegisters()
{
    uint8_t i;
    for (i = 0; i < 0x50; i++)
  printRegister(i);
...
void RH_RF69_PAN::setHeaderPan(uint8_t pan)
{
  _thisPan = pan;
  _txHeaderPan = pan;
}

uint8_t RH_RF69_PAN::headerPan()
{
  return _rxHeaderPan;
}
